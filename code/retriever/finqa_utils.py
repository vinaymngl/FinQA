...
class MathQAExample(
        collections.namedtuple(
            ...
            pre_text post_text table financial_terms"
        )):
...
def wrap_single_pair(tokenizer, question, context,terms, label, max_seq_length,
                    cls_token, sep_token):
    '''
    ...
    if is_training:
        for gold_ind in example.all_positive:
            ...
            this_input_feature = wrap_single_pair(
                tokenizer, question, this_gold_sent,example.financial_terms, 1, max_seq_length,
                cls_token, sep_token)
        ...
        for i in range(len(all_text)):
            if i not in pos_text_ids:
                ...
                this_input_feature = wrap_single_pair(
                    tokenizer, example.question, this_text,example.financial_terms, 0, max_seq_length,
                    cls_token, sep_token)
                this_input_feature["filename_id"] = example.filename_id
        ...
        for this_table_id in range(len(example.table)):
            if this_table_id not in pos_table_ids:
                ...
                this_input_feature = wrap_single_pair(
                    tokenizer, example.question, this_table_line,example.financial_terms, 0, max_seq_length,
                    cls_token, sep_token)
                this_input_feature["filename_id"] = example.filename_id
        ...
        for i in range(len(all_text)):
            this_text = all_text[i]
            this_input_feature = wrap_single_pair(
                tokenizer, example.question, this_text,example.financial_terms, -1, max_seq_length,
                cls_token, sep_token)
            this_input_feature["filename_id"] = example.filename_id
            this_input_feature["ind"] = "text_" + str(i)
            features_neg.append(this_input_feature)
        ...
        for this_table_id in range(len(example.table)):
            ...
            this_table_line = table_row_to_text(example.table[0], example.table[this_table_id])
            this_input_feature = wrap_single_pair(
                tokenizer, example.question, this_table_line,example.financial_terms, -1, max_seq_length,
                cls_token, sep_token)
            this_input_feature["filename_id"] = example.filename_id
            this_input_feature["ind"] = "table_" + str(this_table_id)
            features_neg.append(this_input_feature)
...
def read_mathqa_entry(entry, tokenizer):
    ...
    # financial_terms= entry["financial_terms"]
